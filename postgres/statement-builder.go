...
func (d *StatementBuilder) AddPrefix(name string, prefix squirrel.Sqlizer) *StatementBuilder {
	...
	d.WithPrefix.Add(name, prefix)
	return d
}

// PrefixHasRecursive ensures that the WithPrefix is recursive.
func (d *StatementBuilder) PrefixHasRecursive() *StatementBuilder {
	if d.WithPrefix == nil {
		d.WithPrefix = &With{}
	}
	d.WithPrefix.IsRecursive = true
	return d
}
...
// AddArrayCondition adds a special condition that looks like [column]=ANY(?)
// This does *not* check for an empty value.
func (d *StatementBuilder) AddArrayCondition(column string, value any, xs ...ConditionOption) *StatementBuilder {
	return d.AddCondition(squirrel.Expr(column+"=ANY(?)", value), xs...)
}

// AddArrayCondition allows adding of any array condition. It is assumed that
// the data type of value matches that of the column, and that the data type
// has proper sql.Scanner and driver.Valuer implementations.
func AddArrayCondition[T any](d *StatementBuilder, column string, value []T, xs ...ConditionOption) {
	if len(value) == 0 {
		return
	}
	arr := pq.Array(value)
	d.AddArrayCondition(column, arr, xs...)
}
