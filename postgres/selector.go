...
type Selector[T any] struct {
	...
	PreprocessSelect Preprocessor[squirrel.SelectBuilder]
	...
	// conditions or columns.
	PreprocessGet Preprocessor[squirrel.SelectBuilder]
...
func (s *Selector[T]) Get() (T, error) {
	...
	return Get[T](s, s.Scanner.Scan, s.processGetCols()...)
}
...
func Get[T any](s *Selector[T], scanner ScannerFunc[T], cols ...string) (T, error) {
	...
	if s.PreprocessGet != nil {
		qry = s.PreprocessGet(s.QueryBuilder.from, qry)
	}
...
func (s *Selector[T]) Select() ([]T, error) {
	...
	return Select[T](s, s.Scanner.Scan, s.processGetCols()...)
}
...
func Select[R any](s *Selector[R], scanner ScannerFunc[R], cols ...string) ([]R, error) {
...
func (s *Selector[T]) Iterate() (*SelectIterator[T], error) {
	...
	return Iterate[T](s, s.Scanner.Scan, s.processGetCols()...)
}

// Iterate returns an iterator for retrieving multiple rows sequentially from the database.
func Iterate[R any](s *Selector[R], scanner ScannerFunc[R], cols ...string) (*SelectIterator[R], error) {
	qry := s.QueryBuilder.Builder(cols...).
		PlaceholderFormat(squirrel.Dollar).
		RunWith(s.Provider.GetDb())
...
func (i *SelectIterator[R]) Close() error {
	return i.Rows.Close()
}
